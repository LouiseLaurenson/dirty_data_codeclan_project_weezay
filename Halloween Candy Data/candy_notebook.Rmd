...
I started the cleaning with one data frame at a time, lucky though the 3 data frames and some similar issues.

* **Column names:** The Raw data had alot of issues with columns which had nothing to do with candy, this meant that having to filter out any columns that I found irrelevant to the task. *'e.g "Which day do you prefer, Friday or Sunday?"'* 
I also had to standardize alot of columns over the three data frames. *'e.g your_gender = gender'*
             
* **Age:** The age column in 2015 was as a character string with inputs that needed to cleaned, I made it as an interger column and rounded and .something ages to the nearest whole number, anything that was text input that didnt make sense was changed to NA.
...
* **Year:** As I knew we had to combine the data frames I made sure each data frame had a year column before the join so we dont loses the input year of the data.

* **Pivoit:** To make the data more readable I used a pivot_longer so the data would be easier to do analysis on. Making 'candy'
and 'rating' columns. 
